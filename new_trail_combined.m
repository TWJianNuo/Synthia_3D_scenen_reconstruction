...
function optimize_cubic_shape(rectangular, depth_map)
    ...
    [cuboid, is_valide] = tune_cuboid(cuboid, T*inv(A), P);
    rectangular.cur_cuboid = cuboid;
    sampled_visible_pts = acquire_visible_sampled_points(rectangular, P, T, A);
    [processed_depth_map, lin_ind] = generate_cubic_depth_map_by(cuboid, P, T*inv(A), depth_map, sampled_visible_pts);
    ...
    % fin_param = analytical_gradient_pos_v3(cuboid, P, T*inv(A), processed_depth_map, lin_ind, sampled_visible_pts);
    fin_param = analytical_gradient_combined(cuboid, P, T*inv(A), processed_depth_map, lin_ind, sampled_visible_pts, init_cuboid);
end
...
function [depth_map, linear_ind] = generate_cubic_depth_map_by(cubic, intrinsic, extrinsic, depth_map, visible_pts)
