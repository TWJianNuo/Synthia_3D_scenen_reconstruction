...
for i = 1 : size(help_info, 1)
    [base_path, GT_Depth_path, GT_seg_path, GT_RGB_path, GT_Color_Label_path, cam_para_path, n, save_path] = read_helper_info(helper, i);
    folder_path = make_dir(save_path, base_path); file_ids = create_txt(folder_path);
    for j = 1 : n
        frame = j;
...
function [base_path, GT_Depth_path, GT_seg_path, GT_RGB_path, GT_Color_Label_path, cam_para_path, num_frame, save_path] = read_helper_info(helper, ind)
    base_path = helper{ind,1};
    GT_Depth_path = helper{ind,2};
    GT_seg_path = helper{ind,3};
    GT_RGB_path = helper{ind,4};
    GT_Color_Label_path = helper{ind,5};
    cam_para_path = helper{ind,6};
    num_frame = helper{ind,7};
    save_path = helper{ind,8};
end
