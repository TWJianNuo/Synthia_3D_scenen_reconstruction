function params_end = analytical_gradient_inverse_1(cuboid, intrinsic_param, extrinsic_param, depth_map, linear_ind, sampled_visible_pts)
    % test(cuboid);
    ...
    init_cubic = cuboid;
    activation_label = [1 1 1 0 0 0]; activation_label = (activation_label == 1); it_num = 200; gamma = 0.00000001;
    ...
    for i = 1 : it_num
        ...
        [diff, diff_true] = cal_loss(cuboid, pixel_loc, intrinsic_param, extrinsic_param, gt, plane_ind_batch);
        [sum_diff, sum_hess] = accumulate_delta(cuboid, intrinsic_param, extrinsic_param, pixel_loc, activation_label, gt, plane_ind_batch);
        delta = get_delta_from_diff_and_hess(sum_diff, sum_hess, activation_label);
        params = update_param(params, 0.1 * delta, activation_label);
        if judge_stop(delta, params, diff_record)
            break;
        end
        diff_record(i) = diff; diff_true_record(i) = diff_true;
        % old_params = params; params = update_param(params, delta_theta, activation_label); 
    end
    resulted_cubic = cuboid; 
    figure(2); clf; 
    stem(diff_record(diff_record~=0), 'fill'); figure(3); stem(diff_true_record(diff_true_record~=0), 'fill');
    visualize_re(init_cubic, resulted_cubic, intrinsic_param, extrinsic_param, depth_map);
end
...
function visualize_re(init_cubic, resulted_cubic, intrinsic_params, extrinsic_params, depth_map)
    ...
    figure(1); clf; draw_cuboid(init_cubic); hold on; draw_cubic_shape_frame(resulted_cubic); hold on;
...
function delta_theta = smooth_hessian(sum_diff, sum_hessian, activation_label)
    num_stable = 0; expand_ratio = 10;
    while true
        warning('');
        delta_theta = sum_diff * inv(sum_hessian + eye(sum(activation_label)) * num_stable);
        if length(lastwarn) ~= 0
            if num_stable == 0
                num_stable = 0.000001; 
            else
                num_stable = num_stable * expand_ratio;
            end
        else
            break;
        end
