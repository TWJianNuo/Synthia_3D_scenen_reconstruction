function [pts_2d, depth_store] = find_visible_2d_pts(cubics, pts_2d, intrinsic_params, extrinsic_params)
    ...
    for ii = 1 : num_pt
        ...
        if length(single_pt_all_possible_pos(valid_label)) > 0
            vale_pts = single_pt_all_possible_pos(valid_label, :); depth_local = depth_record(valid_label, :);
            ...
            depth_store(ii) = depth_local(shortest_ind);
        end
    end
    pts_2d = pts_2d(visible_pt_label, :); depth_store = depth_store(visible_pt_label);
end
