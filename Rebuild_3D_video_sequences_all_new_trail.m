...
function objs = seg_image(depth_map, label, instance, extrinsic_params, intrinsic_params)
    ...
    car_label = 8; pdestrian_label = 10; building_label = 2; tree_label = 6; pose_label = 7;
    ...
    for i = 1 : length(existing_instance)
        ...
        linear_ind = sub2ind(size(instance), ix, iy); selector = (label(linear_ind) == car_label | label(linear_ind) == pdestrian_label); linear_ind = linear_ind(selector);
    ...
    label = eliminate_type_pixel(label, min_obj_height, max_obj_height, extrinsic_params, intrinsic_params, depth_map, tree_label);
    label = eliminate_type_pixel(label, min_obj_height, max_obj_height, extrinsic_params, intrinsic_params, depth_map, pose_label);
    for i = 1 : tot_type_num
        if i ~= pose_label && i ~= tree_label && i ~= building_label
            continue
        end
        [ix, iy] = find(label == i);
    ...
    while count < it_length
        count = count + 1;
        if objs{visiting_ind}.type == building_label
            SE = strel('square',2);
    ...
    while count < it_length
        count = count + 1;
        if objs{visiting_ind}.type == tree_label
            SE = strel('square',2);
            objs = further_seg(extrinsic_params, intrinsic_params, [300 300], objs, visiting_ind, SE, min_obj_pixel_num(objs{visiting_ind}.type));
            visiting_ind = visiting_ind - 1;
        end
    ...
    while count < it_length
        count = count + 1;
        if objs{visiting_ind}.type == pose_label
            SE = strel('square',10);
            objs = further_seg(extrinsic_params, intrinsic_params, [300 300], objs, visiting_ind, SE, min_obj_pixel_num(objs{visiting_ind}.type));
            visiting_ind = visiting_ind - 1;
        end
   ...
   %{
    ...
    for i = 1 : length(objs)
        color = rand([1 3]);
        scatter3(objs{i}.new_pts(:,1), objs{i}.new_pts(:,2), objs{i}.new_pts(:,3), 3, color, 'fill')
        hold on
    end
    axis equal
    %}
