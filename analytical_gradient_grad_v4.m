function fin_param = analytical_gradient_grad_v4(cuboid, intrinsic_param, extrinsic_param, visible_pt_3d, depth_map, pts_new, projected_sampled_points)
    ...
    params = generate_cubic_params(cuboid); params(1) = params(1) + 0.1; params(2) = params(2) + 3; rand_depth = rand();
    activation_label = [1 1 1 1 1 0]; activation_label = (activation_label == 1); it_num = 500; diff_record = zeros(it_num, 1);
    delta_record = zeros(it_num, 1); delta = ones(sum(activation_label), 1)' * 0.00005; % path = make_dir();
    for i = 1 : it_num
        sum_grad = get_delta_value(visible_pt_3d, params, extrinsic_param, intrinsic_param, activation_label, depth_map, rand_depth);
        diff = calculate_diff(depth_map, intrinsic_param, extrinsic_param, visible_pt_3d, params, rand_depth);
        diff_record(i) = diff;
    ...
    % visualize_on_depth_map(depth_map, params, visible_pt_3d, extrinsic_param, intrinsic_param)
end
...
function sum_grad = get_delta_value(visible_pt_3d, params, extrinsic_param, intrinsic_param, activation_label, depth_map, rand_depth)
    ...
    for i = 1 : pts_num
        ...
        [grad, diff] = get_grad_value_and_diff(k, plane_ind, params, extrinsic_param, intrinsic_param, activation_label, depth_map, rand_depth);
...
function diff = calculate_diff(depth_map, intrinsic_param, extrinsic_param, visible_pt_3d, params, rand_depth)
    ...
    diff = sum((rand_depth - depth).^2); % depth_map = add_depth_to_depth_map(depth_map, pts2, 30);
...
function [A, diff] = get_grad_value_and_diff(k, plane_ind, params, extrinsic_param, intrinsic_param, activation_label, depth_map, rand_depth)
