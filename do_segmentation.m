...
for frame = 1 : 294
    ...
    output_results(prev_mark, plot_figure, exp_re_path, frame, rgb, extrinsic_params)
end
function output_results(prev_mark, plot_figure, exp_re_path, frame, rgb, extrinsic_params)
    %{
    ...
    imwrite(new_img, [exp_re_path '/_instance_label' num2str(frame) '.png']);
    
    save_intance(prev_mark, frame, exp_re_path)
end
function save_intance(prev_mark, frame, folder_path)
    path = mkdir([folder_path '/Instance_map/']);
    if length(path) == 1
        path = [folder_path '/Instance_map/'];
    end
...
function [max_instance, prev_mark, plot_figure] = seg_image(depth_map, label, instance, prev_mark, extrinsic_params, intrinsic_params, affine_matrx, max_instance, frame)
    ...
    building_type = 2; min_obj_pixel_num = [inf, 400, inf, inf, inf, 70, 10, 10, inf, 10, 10, inf, inf, inf, inf];
    ...
    if isempty(prev_mark)
        ...
        for i = 1 : CC.NumObjects
            ...
            if length(now_found_bindices) > min_obj_pixel_num(building_type)
                [cur_mark{obj_num_count}, max_instance] = init_mark(max_instance, linear_ind_record(now_found_bindices), extrinsic_params, intrinsic_params, affine_matrx, cur_old_pts_set(now_found_bindices, :), rand([1 3]), cur_new_pts_set(now_found_bindices, :), frame, align_matrix);
        ...
        for i = 1 : CC.NumObjects
            ...
            if length(now_found_bindices) > min_obj_pixel_num(building_type)
                ...
                if length(unique_found_instance_id) == 1 && unique_found_instance_id(1) == 0
                    ...
                    [cur_mark{obj_num_count}, ~] = init_mark(father_instanceId, linear_ind_record(now_found_bindices), extrinsic_params, intrinsic_params, affine_matrx, cur_old_pts_set(now_found_bindices, :), find_color(father_instanceId, prev_mark), cur_new_pts_set(now_found_bindices, :), frame, align_matrix);
                end
                obj_num_count = obj_num_count + 1;
            end
        end
...
function [cur_mark, max_instance] = init_mark(max_instance, linear_ind, extrinsic_params, intrinsic_params, affine_matrx, pts_old, color, pts_new, frame, adjust_matrix)
