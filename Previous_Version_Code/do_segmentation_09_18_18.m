...
function save_intance(prev_mark_record, image_size)
    ...
    for i = 1 : length(prev_mark_record)
        ...
        obj = prev_mark_record{i};
        % for j = 1 : length(obj)
            % img(obj{j}.linear_ind) = obj{j}.instanceId;
        % end
        full_path = [path num2str(i, '%06d') '.mat'];
        save(full_path, 'obj');
    end
end
        ...
        for i = 1 : CC.NumObjects
            ...
            if length(now_found_bindices) > min_obj_pixel_num(building_type)
                ...
                if length(unique_found_instance_id) == 1 && unique_found_instance_id(1) == 0
                    ...
                    [cur_mark{obj_num_count}, max_instance] = init_mark(father_instanceId, linear_ind_record(now_found_bindices), extrinsic_params, intrinsic_params, affine_matrx, cur_old_pts_set(now_found_bindices, :), find_color(father_instanceId, prev_mark), cur_new_pts_set(now_found_bindices, :));
                    obj_num_count = obj_num_count + 1;
                end
            end
        end
