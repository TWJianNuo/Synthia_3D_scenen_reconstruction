...
function objs = seg_image(depth_map, label, instance, extrinsic_params, intrinsic_params)
    ...
    pose_type = 7; label = eliminate_type_pixel(label, min_obj_height, max_obj_height, extrinsic_params, intrinsic_params, depth_map, pose_type);
    ...
    while count < it_length
        count = count + 1;
        if objs{visiting_ind}.type == pose_type
            SE = strel('square',25);
